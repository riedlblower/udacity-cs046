...
public class Connect4Game
{
    ...
    /**
     * Check if the one player has three in a row and nearly won.
     * 
     * @return 'R' if red has three in a row, 'Y' if yellow has three in a row, 'N' if no player has three in a row.
     */
    public int threeInARow(char colour)
    {
        char[][] board = getBoardMatrix();
        int blankSlotCounter = -1;
        int nextWinningColumn = -1;
        for (int i = 0; i < getColumnCount(); i++)
        {
            for (int j = 0; j < getRowCount(); j++)
            {
                String fourSlots = "";
                    if (j + 3 < getRowCount())
                    {
                        fourSlots = ""+board[j][i] + board[j + 1][i] + board[j + 2][i] + board[j + 3][i];  
                        blankSlotCounter = threeOutOfFourSlots(fourSlots, colour);
                        if(blankSlotCounter > -1)
                        {
                            // no need to check for empty slot in vertical column of slots
                            highlightSlot(j, i);
                            highlightSlot(j + 1, i);
                            highlightSlot(j + 2, i);
                            highlightSlot(j + 3, i);
                            nextWinningColumn = i;
                            return nextWinningColumn;
                        }
                    }
                    if (i + 3 < getColumnCount())
                    {
                        fourSlots = ""+board[j][i] + board[j][i + 1] + board[j][i + 2] + board[j][i + 3];  
                        blankSlotCounter = threeOutOfFourSlots(fourSlots, colour);
                        if (blankSlotCounter > -1)
                        {
                            if (j < getRowCount() - 1)  //no need to check on lowest row, hence "-1"
                            {
                                if(checkSlotUnderneathFull(i+blankSlotCounter,j))
                                {
                                    highlightSlot(j, i);
                                    highlightSlot(j, i + 1);
                                    highlightSlot(j, i + 2);
                                    highlightSlot(j, i + 3);
                                    nextWinningColumn = i + blankSlotCounter;
                                    return nextWinningColumn;
                                }
                            }
                        }
                    ...
                    if (i + 3 < getColumnCount() && j + 3 < getRowCount())
                    {
                        fourSlots = ""+board[j][i] + board[j + 1][i + 1] + board[j + 2][i + 2] + board[j + 3][i + 3];  
                        blankSlotCounter = threeOutOfFourSlots(fourSlots, colour);
                        if(blankSlotCounter > -1)
                        {
                            if (j + 2 + blankSlotCounter <= getRowCount())
                            {
                                if (checkSlotUnderneathFull(i + blankSlotCounter,j + blankSlotCounter))
                                {
                                    highlightSlot(j, i);
                                    highlightSlot(j + 1, i + 1);
                                    highlightSlot(j + 2, i + 2);
    ...
    public int threeOutOfFourSlots(String fourSlots, char colour)
    {
        ...
        if (colour == 'R')
        {
            switch (fourSlots)
            {
                case "BRRR": blankSlotCounter = 0; break;
                case "RBRR": blankSlotCounter = 1; break;
                case "RRBR": blankSlotCounter = 2; break;
                case "RRRB": blankSlotCounter = 3; break;
            }
        }
        else if (colour == 'Y')
        {
            switch (fourSlots)
            {
                case "BYYY": blankSlotCounter = 0; break;
                case "YBYY": blankSlotCounter = 1; break;
                case "YYBY": blankSlotCounter = 2; break;
                case "YYYB": blankSlotCounter = 3; break;
            }
        }
        return blankSlotCounter;
    }

    
    public boolean checkSlotUnderneathFull(int i, int j)
    {
        if (getColumn(i).getSlot(j+1).getIsFilled())
        {
            return true;
        }
        else
